...
server <- function(input, output) {
  ...
  output$plot_treatment<-renderPlot(
    ...
    ggplot(data=data3 %>% dplyr::filter(STATE==input$State_Name),
           aes(A_PCT, B_PCT)) + 
      geom_point(color='red')+labs(title="Treated for Alcohol and Drug Abuse vs. Treated for Alcohol Abuse", x="Percent of clients treated for only alcohol abuse", y="Percent of clients treated for both alcohol and drug abuse")
